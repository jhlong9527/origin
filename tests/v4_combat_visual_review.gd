...
func _shot(filename: String) -> void:
	...
	var error := picture.save_png(ProjectSettings.globalize_path("res://captures/v4_" + filename + ".png"))
...
func _run() -> void:
	...
	if not samples.is_empty():
		...
		var report := FileAccess.open("res://tests/v4_render_metrics.json", FileAccess.WRITE)
		report.store_string(JSON.stringify(result, "\t"))
		print("V4_RENDER_METRICS ", JSON.stringify(result))
	print("V4_VISUAL_REVIEW ", "PASS" if failures == 0 else "FAIL")
	main.free()
	quit(failures)
