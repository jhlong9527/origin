...
func _shot(filename: String) -> void:
	...
	var error := picture.save_png(ProjectSettings.globalize_path("res://captures/v5_" + filename + ".png"))
...
func _reset(player_at := Vector3(-2, .03, 2), boss_at := Vector3(1, .03, -1)) -> void:
	...
	# This deterministic effects review explicitly opts into the optional lock.
	# Free mouse aim is covered separately by physical-cursor integration checks.
...
func _run() -> void:
	...
	while main.combat.boss_time < .90:
		await physics_frame
	...
	if not samples.is_empty():
		...
		var report := FileAccess.open("res://tests/v5_render_metrics.json", FileAccess.WRITE)
		report.store_string(JSON.stringify(result, "\t"))
		print("V5_RENDER_METRICS ", JSON.stringify(result))
	print("V5_VISUAL_REVIEW ", "PASS" if failures == 0 else "FAIL")
	main.free()
	quit(failures)
